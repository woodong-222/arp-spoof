...
int main(int argc, char *argv[])
{
	...
	for (int i = 2; i < argc; i += 2)
	{
		...
		printf("Info: Successed to Sent ARP Reply\n");
		...
		while(true)
		{
			...
			if (eth_hdr->smac() == sender_mac && eth_hdr->type() == EthHdr::Ip4) 
			{
				time_t now = time(0);
				...
				printf("IPv4 Header:\n");
				printf("  - Protocol: %d (%s)\n", ip_hdr->protocol(), 
					ip_hdr->protocol() == Ipv4Hdr::Tcp ? "TCP" :
					ip_hdr->protocol() == Ipv4Hdr::Udp ? "UDP" :
					ip_hdr->protocol() == Ipv4Hdr::Icmp ? "ICMP" : "Other");
				...
				if (send_res != 0) {
					fprintf(stderr, "*Error: forward packet: %s\n", pcap_geterr(pcap));
				} else {
					count++;
				}
			...
			if (eth_hdr->type() == EthHdr::Arp) 
			{
				...
				if (ntohs(arp_hdr->op_) == ArpHdr::Reply && ntohl(arp_hdr->sip_) == Ip(sender_ip) && arp_hdr->smac_ == sender_mac) 
				{
					...
					spoof_packet.arp_.sip_ = htonl(Ip(target_ip));
					spoof_packet.arp_.tmac_ = sender_mac;
					spoof_packet.arp_.tip_ = htonl(Ip(sender_ip));
					...
					printf("Info: Re-spoofed ARP table for %s\n", sender_ip);
				}
			}
		}
	}
